...
const int numexperiments = 500; // should be about 5000
...
int reweighted_pseudoexp()
{
  ...
  bool approxMeas = false;
  bool donotreweight = false;


/*  
  string env_approx = getEnv("APPROX_MEAS");
  if(env_approx.find("TRUE") != string::npos) approxMeas = true;
  else if(env_approx.find("FALSE") != string::npos) approxMeas = false;
  else
  {
...
*/
  ...
  // init reweighted reconstructed histos
    ...
    if(!random_split)
    {
      ...
      //      file_ttbar_mu = new TFile(sl_ttbarele);
    }
    else
    {
  ...
  for(int i=0; i<1; i++)
  {
    ...
    for(int j=nentries_toskip; j<nentries_toconsider; j++)
    {
      ...
      {
        if(discont)
          xgen = myh2gen.getXBinCenter(myh2gen.findXBin(xgen));
...
/*      
      if(approxMeas)
      {
        contk = kApproxMeas[myh2gen.findXBin(fabs(xgen))];
        contweight = (1+contk*diffabsetagen);
      }
*/
    ...
    for(int i=0; i<5; i++)
    {
	  ...
	  //        case 1:  h=hrecttbar_rew_mu; h1d=hrecttbar_rew_mu1d; nfit=nttbarmu; fiterror=0; break;      
        ...
        default: const int tempi = i-1;
          ...
          h1d=myunfold1d.bgwrappedlist_simplebins[tempi];
...
/*  
  if(approxMeas)
    reweightname = "reweightedpseudo_approxmeas";
*/
//  else if(zprime)
  if(zprime)
  ...
  TString outname = TString("output/")+reweightname+rewvar;
  ...
  ofstream resfile(TString("output/")+fileSuffix+"/reweightings/"+rewvar+"_"+reweightname+".txt");


  cout << "fileSuffix: " << fileSuffix << endl;
  cout << "rewvar: " << rewvar << endl;
  cout << "reweightname: " << reweightname << endl;
